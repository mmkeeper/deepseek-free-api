...
tool_coll_header += "Имена и описания параметров каждого инструмента содержатся в формате JSON в описании каждого не отложенного инструмента. Описания параметров отложенных инструментов загружаются через tool_describe" + chr(10)
tool_coll_header += "Значения параметров при вызове инструмента выводятся в формате JSON или обычным текстом (как указано в описании конкретного инструмента) в тегах <usr_parameter name=\"ИМЯ_ПАРАМЕТРА\">ЗНАЧЕНИЕ</usr_parameter>" + chr(10) + chr(10)
...
tool_coll_header += chr(10)
tool_coll_header += "Шаг 3. Вызов — только вне размышлений и только внутри блока:" + chr(10)
tool_coll_header += "```xml" + chr(10)
tool_coll_header += "XML" + chr(10)
tool_coll_header += "```" + chr(10)
tool_coll_header += "Скопируй проверенный XML ПОСИМВОЛЬНО, без единого изменения, и вставь его в блок ```xml ... ```. Внутри блока помещай только XML, без пояснений." + chr(10)
tool_coll_header += "Если размышления выключены — черновик и проверку выполни мысленно, вызов всё равно выводится в блоке ```xml ... ```." + chr(10)
