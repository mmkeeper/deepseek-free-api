...
export function extractDeltaText(value, cache, eventName = "") {
  ...
  function visit(node, path) {
    ...
    if (typeof node.response_message_id === "number") messageId = node.response_message_id;
    ...
    if (node.o === "APPEND" && typeof node.p === "string" && node.p.endsWith("/content") && typeof node.v === "string") {
      text += node.v;
      return;
    }
    ...
    if (typeof node.content === "string" && ["RESPONSE", "TEMPLATE_RESPONSE", "THINK"].includes(node.type)) {
    ...
    if (typeof node?.choices?.[0]?.delta?.content === "string") text += node.choices[0].delta.content;
