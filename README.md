...
| `--no-thinking` | _(выкл)_ | Отключить мышление по умолчанию |
| `--no-search` | _(выкл)_ | Отключить поиск по умолчанию |
| `--login` | - | Войти через Playwright |
| `--connect` | - | Подключиться к Chrome через CDP |
| `--manual` | - | Инструкция по ручному экспорту |

### Параметры сессии DeepSeek

В каждом запросе можно переопределить настройки сессии:
